...
if v:version > 703 || v:version == 703 && has('patch1170')
  function! vital#_vital_complete#Vim#ScriptLocal#import() abort
    return map({'scriptnames': '', 'sid2path': '', 'sid2sfuncs': '', 'sfuncs': '', 'svars': '', 'sid2sfuncs_info': '', 'sid': '', 'sid2svars': ''},  'function("s:" . v:key)')
  endfunction
else
  function! s:_SID() abort
    return matchstr(expand('<sfile>'), '<SNR>\zs\d\+\ze__SID$')
  endfunction
  execute join(['function! vital#_vital_complete#Vim#ScriptLocal#import() abort', printf("return map({'scriptnames': '', 'sid2path': '', 'sid2sfuncs': '', 'sfuncs': '', 'svars': '', 'sid2sfuncs_info': '', 'sid': '', 'sid2svars': ''}, \"function('<SNR>%s_' . v:key)\")", s:_SID()), 'endfunction'], "\n")
  delfunction s:_SID
endif
" ___vital___
...
function! s:sid2sfuncs_info(sid) abort
  ":h :function /{pattern}
  " ->         ^________
  "    function <SNR>14_functionname(args, ...)
  let fs = s:_capture_lines(':function ' . printf('/^%s%s_', s:SNR, a:sid))
  ...
  "    function <SNR>14_functionname(args, ...) abort dict
  let pattern = printf('\m^function\s<SNR>%d_\(\w\{-}\)(\([^)]\{-}\))\%( \(.*\)\)\=$', a:sid)
  for list in map(fs, 'matchlist(v:val, pattern)')
    let [_, funcname, arg_str, atr_str; __] = list
    let r[funcname] = {
    \   'func': function(s:_sfuncname(a:sid, funcname)),
    \   'args': split(arg_str, ', '),
    \   'attrs': split(atr_str, ' '),
    \ }
  endfor
